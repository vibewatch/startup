...
const hedgeRules = [
  ...
  },
  {
    ...
    excludeContext: new RegExp(`${insuranceClaimNouns.source}|\\bclaims\\s+in\\s+this\\s+table\\s+are\\s+minted\\s+locally\\b|\\bpublic[- ]claims?\\s+dispersion\\b|\\bwarranty\\s+claims?\\b(?=\\s*(?:[,.;:]|$)|\\s+(?:history|rates?|response|frequency|data|targets?|resolved|triggered|processing|handling|costs?|by|if|and|or)\\b)`, 'gi'),
