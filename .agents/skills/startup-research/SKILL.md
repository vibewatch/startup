...
Workflow narrative: what to run, in what order, with which flags. Two generated companions carry the static reference material. Load them selectively instead of spending context on validator catalogs before a failure exists:

- [`references/rules.md`](references/rules.md) — read *Agent policy*, *Gates*, *ID system*, and *Renderer contracts* before authoring. Read the named validator dimension only after a check reports it.
- [`references/contracts.md`](references/contracts.md) — chapter workers read *Analysis chapter shape*; the orchestrator reads *Report meta shape* before finalization. Read runtime/envelope sections only when using those outputs.
...
5. **Research profile (before chapter loading):** `npm run research:profile -- --report-folder <reportFolder> --profile fast|deep`. `fast` resolves the configured time-bounded evidence, exhibit, and depth floors into the report snapshot; `deep` snapshots the unchanged repository defaults. The command is idempotent for the same profile, refuses profile changes and finalized reports, and defaults to `deep`. Automation must pass `fast` explicitly.
...
1. Load its per-chapter delta:
   ...
   - Pass the JSON directly to the worker or save it only under `.research-cache/<runId>/`. Never write runtime-context JSON into the report folder.
...
2. Select candidates from the run-level search bundle → fetch → record (run this **before** step 3 so `localEvidence` is populated when the YAML body is written):
   - Read `.research-cache/<runId>/search-bundle.json` → `chapterPools[]` for the current chapter. Start with successful prefetched `recommended[]` candidates (`allocation: net-new` and `report-diversity`, then `chapter` and `shared`); each candidate's `fetch.outputFile` points to reviewed text. Retain at least `chapter.gate.minNetNewSources` relevant successful `net-new` candidates so convergence does not fail after sibling chapters land. If a marked net-new candidate is irrelevant, replace it with a relevant reserve URL not used by another pool rather than silently filling with another shared URL. Retain only evidence that actually supports the chapter. Fetch from `reserve[]` only when successful prefetched evidence cannot satisfy the gate.
   - To inspect or rerun only the plan without executing searches:
     `npm run research:plan -- --report-folder <reportFolder> --company "<companyName>" [--website "<companyUrl>"] [--chapter <key|order>] [--profile fast|deep]`
   - Execute discovery queries through the cached provider-neutral runner:
     ...
     `auto` follows each query's preferred/fallback provider order. AnySearch supports its anonymous in-memory credential handoff; configured Brave, Exa, or Tavily keys remain optional fallbacks. Credentials are never written to the search bundle or cache. Named-provider mode requires that provider's key when it does not support anonymous access: `ANYSEARCH_API_KEY`, `BRAVE_SEARCH_API_KEY`, `TAVILY_API_KEY`, `EXA_API_KEY`.
   ...
   - Never cite this site's own reports (`startup.genisisiq.com`, including its subdomains) as evidence. Discovery excludes these URLs from fresh and cached results and records `excludedResults`; bootstrap records `rejectedSelfPublishedCount` and treats redirects back to the site as failed evidence, allowing reserve recovery. Workers refuse old pools that still mark self-published URLs or redirects as successful; rerun bootstrap before starting them. The report content gate rejects direct self-citations with `circularReportSource`; fast finalization also rejects self-published URLs or redirects in old successful bundles. Repair the dependent claims from original external sources, not by relabeling the circular source. Historical `--contract` checks remain structural and do not certify source independence.
...
> **Parallel worker contract.** Launch each worker with the exact `runtimeContext.policy.workerRouting.model` and `runtimeContext.policy.workerRouting.reasoningEffort`; top-level CLI model/effort settings do not reliably propagate to subagents. Pass each worker the `reportFolder`, `runId`, its chapter `order`/`key`/`file`/`letter`, the full `runtimeContext` (including `runtimeContext.policy.retryPolicy` and `workerRouting`), its single `search-bundle.json.chapterPools[]` entry, and `STARTUP_FETCH_LOG_PATH` (absolute path if any worker may chdir). Fast workers may cite only successful prefetched URLs already present in `search-bundle.json.fetchedSources`; do not fetch, add, or manually log a new URL during drafting or convergence. Deep workers may fetch a reserve candidate only when successful allocated evidence cannot clear a named gate. Retain all relevant `net-new` entries needed by `gate.minNetNewSources` plus the pool's relevant `independent-candidate` entries so report-level domain and independence gates pass without late repair. Do not inspect historical reports for examples—the runtime context, contracts, and renderer rules are the complete authoring surface. Each worker may write only `reportFolder/<runtimeContext.chapter.file>` and scratch under `.research-cache/<runId>/`; no sibling-chapter, `report-meta.yaml`, assembled-artifact, or git edits. Each worker must return: `chapter.file`, `normalStatus` (`passed`/`failed`), `normalRetries`, `strictStatus` (`skipped` for transient-only deferral, else `passed`/`failed`), `strictRetries`, `survivingFailureDimensions[]`. The orchestrator waits for every worker, verifies every configured chapter file exists, then runs the convergence sweep before authoring `report-meta.yaml`.
...
3. **Run finalization:**
   ...
   - **Workflow-config snapshot:** `research:profile` normally writes the resolved fast/deep snapshot before chapter work. If no snapshot exists, finalize-report freezes the current deep head config on first run. Every re-run reuses the snapshot, so later config edits never retroactively re-judge the report. Pass `--refresh-snapshot` only when intentionally repairing an old report against the current deep head config.
   - **Fast-source provenance runs on every finalization attempt**, including retries and preassembled reports. Retain `.research-cache/<runId>/search-bundle.json` through all workflow finalization passes and retries; an existing `summary-card.yaml` does not prove the inputs are valid. Every cited URL must have a successful prefetch and belong to that chapter's successful recommended or reserve pool. Missing provenance fails explicitly; historical contract-only checks remain available through `check:reports-contract` without a research cache.
   ...
   - **Exit 4** (pre-flight): the report folder or `report-meta.yaml` is missing, scratch/context files were written into a new report folder, or a configured chapter is missing when the strict sweep starts. Move scratch files under `.research-cache/<runId>/`, fix the missing authored artifact, and rerun.
